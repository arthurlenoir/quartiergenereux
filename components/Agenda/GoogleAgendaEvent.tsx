...
const CalendarEvent: React.FC<Props> = ({ event, link, preview = false }) => {
    ...
    return <div className={styles.event}>
        <div className={styles.eventDate}>
            ...
            <div className={styles.eventDateDay}>{start.toLocaleDateString(localTime, dateRenderOptions)}</div>
            <div className={styles.eventDateMonth}>{getShortMonthLabel(start)}</div>
        </div>
        <div className={styles.eventContent}>
            <div className={styles.eventTitle}>{event.summary}</div>
            <div className={styles.eventDescription} dangerouslySetInnerHTML={{ __html: event.description }} />
            <div className={styles.eventTime}>{start.toLocaleTimeString(localTime, timeRenderOptions)} - {end.toLocaleTimeString(localTime, timeRenderOptions)}</div>
        </div>
    </div>;
};
